...
#include "../core/element_result.hpp"
#include <vector>
#include <string>
#include <chrono>
 ...
 * Handles display and formatting of UI elements in various output formats
 */
class ElementCLI {
public:
    ElementCLI();
...
private:
    ...
    std::string elementTypeToDisplayString(ElementType type) const;
    std::string elementStateToDisplayString(ElementState state) const;

    // Table formatting helpers
    void printElementTableHeader() const;
    void printElementTableRow(const UIElement& element) const;
    void printElementSeparator() const;

    // JSON formatting helpers
    std::string formatElementsAsJsonArray(const std::vector<UIElement>& elements) const;
